...
目前不均衡数据对于建模的影响比较小，第一是分类场景比较少；第二是已经在模型中采用了较好的方式。具体的做法可以见下列处理。
...
32. PSI(population stability index):可以用于特征\模型的时间稳定性测定
...
lightGBM算法是不需要进行连续型变量的分箱，因为会自动设定buckets.
...
- 很明显，bagging可以降低方差，boosting可以降低偏差,并不会影响方差。
...
参见陈天奇论文：
| no | XGBOOST\(GBDT工程实现\)                                          | GBDT                           |
|----|--------------------------------------------------------------|--------------------------------|
| 1  | 代价函数中添加了正则项，用来防止过拟合                                          | NA                             |
